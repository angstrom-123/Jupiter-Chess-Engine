...
// Higher quality book than original
...
#define READ_U64(bytes, offset) \
        ...
        | (static_cast<uint64_t>(bytes[(offset) + 4]) << 24ul) \
...
OpeningBook::OpeningBook()
{
    ...
    m_Bytes.resize(m_FileSizeBytes);
    file.read(reinterpret_cast<char *>(m_Bytes.data()), m_FileSizeBytes);
...
bool OpeningBook::LookupMoves(const BoardState& state, OpeningMoves& moves) const
{
    ...
    uint64_t low = 0;
    uint64_t high = m_FileSizeBytes / POLYGLOT_ENTRY_SIZE;
    uint64_t found = UINT64_MAX;
    while (low <= high) {
        uint64_t mid = (high + low) / 2;
        uint64_t midOffset = mid * POLYGLOT_ENTRY_SIZE;;
        ZobristKey midKey = READ_U64(m_Bytes, midOffset);
    ...
    // Couldn't find this position in the book
    if (found == UINT64_MAX)
        return false;

    // There are many entries with the same key in a row (alternative moves) so need to find first
    uint64_t firstIndex = found;
    while (found > 0) {
        uint64_t offset = (firstIndex - 1) * POLYGLOT_ENTRY_SIZE;
        if (READ_U64(m_Bytes, offset) != targetKey)
            break; // Reached the first entry in the block
        firstIndex--;
    }

    // Now scan forward to find all matching entries
    while (moves.Size() < moves.Capacity()) {
        uint64_t offset = firstIndex * POLYGLOT_ENTRY_SIZE;
        if (READ_U64(m_Bytes, offset) != targetKey)
...
std::pair<Move, uint16_t> OpeningBook::ParseMove(const BoardState& state, uint64_t bits) const
{
    ...
    // Move
    const uint16_t moveBits = (bits & 0xFFFF000000000000) >> 48;
    uint8_t toFile   = (moveBits & 0b0000000000000111);
    uint8_t toRow    = (moveBits & 0b0000000000111000) >> 3;
    ...
    uint8_t toIndex = ToIndex(toFile, 7 - toRow);
    Move move = {
        .from = fromIndex,
        .to = toIndex,
    ...
    uint16_t weight = weightBits;

    // Learn (unused for now)
    const uint16_t learnBits = bits & 0x00000000FFFFFFFF;
    (void) learnBits;
...
ZobristKey OpeningBook::ZobristHash(const BoardState& state) const
{
    ...
    // Turn to move (polyglot checks for white)
